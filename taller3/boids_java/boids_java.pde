...
class Flock {
  ...
  void run() {
    for (Boid b : boids) {
      b.run(boids);  // pasar toda la lista de boids a cada boid de manera indivicual
    }
  }
...
class Boid {
  ...
  Boid(float x, float y, float z) {
    ...
    velocity = new PVector(cos(angle), sin(angle) * cos(angle), sin(angle));
